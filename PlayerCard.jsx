...
function getEvRole(player) {
  const pos = (player.position || "").toUpperCase();
  const avgToi = parseAvgToi(player.toi);
  if (pos === "D") {
    if (avgToi >= 23) return "1st Pair";
    if (avgToi >= 20) return "Top 4";
    if (avgToi >= 17) return "2nd Pair";
    return "3rd Pair";
  }
  if (avgToi >= 20) return "1st Line";
  if (avgToi >= 17) return "Top 6";
  if (avgToi >= 14) return "Middle 6";
  return "Depth";
}
...
function PercentileCardView({ player, accent, age, teamAbbr, teamFull }) {
  ...
  const foPctRaw = foTotal >= 20
    ? ((player.fow || 0) / foTotal * 100)
    : (player.fo_pct != null ? player.fo_pct : null);
  const foPct = isCenter ? foPctRaw : null;

  const toiPP = player.toi_pp || 0;
  const toiPK = player.toi_pk || 0;
  const gpForST = player.gp || 1;
  const hasPP = (toiPP / gpForST) >= 0.5;
  const hasPK = (toiPK / gpForST) >= 0.17;
  const warTrend = player.warTrend || [];
  ...
  const warTiles = [
    ...
    { label: "PP", value: hasPP ? percentiles["PP"] : null, na: !hasPP },
    { label: "PK", value: hasPK ? percentiles["PK"] : null, na: !hasPK },
  ...
  const contextBars = [
    ...
    ...(isCenter ? [{ label: "Faceoff % \u2746", value: percentiles["FO%"] ?? null, rawPct: foPct }] : []),
  ];
    ...
    <div style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "55% 1fr", gap: 24, paddingTop: 16, borderLeft: `3px solid ${accent}33`, paddingLeft: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "flex-start", gap: 0, background: `linear-gradient(135deg, ${accent}08 0%, transparent 60%)`, borderRadius: 8, padding: "8px 10px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {contextBars.map((bar) => {
              ...
              let displayVal, barWidth, barColor;
              if (isFO) {
                if (bar.value != null) {
                  displayVal = Math.round(bar.value);
                  barWidth = bar.value;
                  barColor = pc(bar.value);
                } else if (bar.rawPct != null) {
                  displayVal = `${bar.rawPct.toFixed(1)}%`;
                  barWidth = bar.rawPct;
                  barColor = "#f0c040";
                } else {
                  displayVal = "\u2014";
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <TrendPanel
            title=""
            subtitle={<span><span style={{ color: "#2fb4ff" }}>EV Off</span> vs <span style={{ color: "#ff6b6b" }}>Def</span> vs <span style={{ color: "#35e3a0" }}>Finish</span></span>}
            data={impactTrend}
            lines={[
              { key: "off", label: "EV Off", color: "#2fb4ff" },
              { key: "def", label: "EV Def", color: "#ff6b6b" },
              { key: "finishing", label: "Finishing", color: "#35e3a0" },
            ]}
            accent={accent}
          />
      ...
      {/* ── Footer watermark ─────────────────────────────────────────────── */}
      <div style={{ marginTop: 20, paddingTop: 14, borderTop: "1px solid rgba(255,255,255,0.05)", textAlign: "center", fontSize: 8, color: "#2a3a4a", fontFamily: "'DM Mono',monospace", letterSpacing: "0.14em", textTransform: "uppercase" }}>
        {"3-Year Weighted Avg \u00b7 Percentile Rank Among "}{positionLabel.charAt(0).toUpperCase() + positionLabel.slice(1)}{" \u00b7 NHL Analytics"}
      </div>
    </div>
  );
}
...
function PlayerCard({ player }) {
  ...
  const cardWidth = !isGoalie && tab === "percentile card" ? 1080 : 900;

  // Computed for the percentile card header season-stat boxes
  const pcIsCenter = !isGoalie && (player.position || "").toUpperCase() === "C";
  const pcFoTotal = (player.fow || 0) + (player.fol || 0);
  const pcFoPctRaw = pcFoTotal >= 20
    ? ((player.fow || 0) / pcFoTotal * 100)
    : (player.fo_pct != null ? player.fo_pct : null);
  const pcFoPct = pcIsCenter ? pcFoPctRaw : null;
  const pcAvgToi = parseAvgToi(player.toi);
  const pcIxgPer60 = player.ixg != null && gp > 0 && pcAvgToi
    ? player.ixg / (pcAvgToi * gp / 60) : null;
  const headerSeasonStats = [
    { label: "POINTS", value: player.pts != null ? String(player.pts) : "\u2014", color: null },
    { label: "GOALS", value: player.g != null ? String(player.g) : "\u2014", color: null },
    { label: "AVG TOI", value: player.toi || "\u2014", color: "#35e3a0" },
    {
      label: pcIsCenter && pcFoPct != null ? "FACEOFF %" : "ixG / 60",
      value: pcIsCenter && pcFoPct != null
        ? `${pcFoPct.toFixed(1)}%`
        : (pcIxgPer60 != null ? pcIxgPer60.toFixed(2) : "\u2014"),
      color: "#2fb4ff",
    },
  ];
  const warPct = player.percentiles?.["WAR"] ?? player.overall_rating ?? null;
  ...
  }, [careerStats, player.player_id, tab]);
  ...
  return (
    <div className="pc-card" style={{ width:cardWidth, background:"#05090f", borderRadius:18, border:"1px solid rgba(255,255,255,0.08)", overflow:"hidden", boxShadow:`0 0 0 1px #0a1520,0 24px 60px rgba(0,0,0,0.6),0 0 80px ${accent}15`, fontFamily:"'Barlow Condensed',sans-serif", position:"relative" }}>
      ...
      <div style={{ padding:"16px 20px", background:`linear-gradient(135deg,${accent}22 0%,transparent 60%)`, borderBottom:"1px solid #1a2535", position:"relative", overflow:"hidden" }}>
        ...
        <div className="pc-header-row" style={{ display:"flex", gap:16, alignItems:"flex-start", position:"relative", zIndex:1 }}>
          {/* Headshot */}
          ...
          {/* Season stat boxes — shown only on percentile card tab */}
          ...
          {/* WAR / position badge */}
          <div className="pc-war-badge" style={{ background:"#00e5a015", border:"1px solid #00e5a044", borderRadius:10, padding:"14px 20px", textAlign:"center", minWidth:130, flexShrink:0 }}>
            {isGoalie ? (
              <>
              ...
              <>
                <div style={{ fontSize:36, fontWeight:900, color:"#00e5a0", lineHeight:1 }}>{player.war_total != null ? `${player.war_total > 0 ? "+" : ""}${player.war_total.toFixed(1)}` : "\u2014"}</div>
                ...
                <div style={{ marginTop:6, height:3, background:"rgba(53,227,160,0.15)", borderRadius:2 }}>
                  <div style={{ height:"100%", width:`${warPct ?? 0}%`, background:"#35e3a0", borderRadius:2 }} />
                </div>
                <div style={{ fontSize:9, color:"#35e3a0", opacity:0.7, marginTop:3, fontFamily:"'DM Mono',monospace" }}>
                  {warPct != null ? `${Math.round(warPct)}th percentile` : "\u2014"}
                </div>
              </>
            )}
          </div>
      ...
      <div style={{ padding:"18px 20px 20px" }}>
        ...
        {!isGoalie && tab === "ratings" && (
          <div>
            {/* Explanation legend */}
            <div style={{ background: "#0d1825", border: "1px solid #1a2d40", borderRadius: 8, padding: "12px 14px", marginBottom: 20 }}>
              <div style={{ fontSize: 10, color: "#3a5a78", fontFamily: "'DM Mono',monospace", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10 }}>How Ratings Work</div>
              <div style={{ display: "grid", gap: 6 }}>
                {[
                  { label: "Overall", desc: "Weighted blend of offensive and defensive ratings, adjusted for position and PP usage." },
                  { label: "Offensive", desc: "Built from ixG/60, primary assists, pts/60, shot creation (iCF/60), xGF%, and RAPM offence." },
                  { label: "Defensive", desc: "Built from xGF%, HDCF%, shot suppression (CF%), TKA rate, GVA (inverted), PK usage, and RAPM defence." },
                ].map(({ label, desc }) => (
                  <div key={label} style={{ display: "flex", gap: 8, alignItems: "baseline" }}>
                    <span style={{ fontSize: 10, color: "#6a8aa8", fontFamily: "'DM Mono',monospace", minWidth: 68, textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</span>
                    <span style={{ fontSize: 11, color: "#7a8a9a", lineHeight: 1.5 }}>{desc}</span>
                  </div>
                ))}
              </div>
            </div>
...
function enrichPlayersWithSeasonTrends(players, seasonStats) {
  ...
  seasons.forEach((season) => {
    ...
    ["F", "D", "G"].forEach((group) => {
      ...
      const allRows = seasonLookups[season][group] || [];
      ...
      seasonLookups[season][`${group}_finishing`] = computePercentileLookup(allRows, "war_shooting");
  ...
  return players.map((player) => {
    ...
    seasons.forEach((season) => {
      ...
      const finishingPct = seasonLookups[season][`${group}_finishing`]?.[player.player_id] ?? null;

      if (warPct != null) {
        warTrend.push({ season, war: warPct });
      }
      if (offPct != null || defPct != null || finishingPct != null) {
        impactTrend.push({ season, off: offPct ?? null, def: defPct ?? null, finishing: finishingPct ?? null });
      }
    });
