...
Loads 18 seasons of Evolving Hockey data (07-08 → 25-26), fits asymmetric
age curves separately for forwards and defensemen using scipy curve_fit,
then computes market value, surplus value, and 7-year WAR trajectories for
all current players.
...
COVID_SEASON      = '20-21'
COVID_WEIGHT      = 0.68            # 56-game season → 0.68 weight

# Market value: $3.15M per WAR point above replacement (linear).
# Calibrated so that the top RAPM-WAR player (~3.7 WAR) is worth ~$12.5M,
# which aligns with elite max contracts (McDavid anchor).
# The RAPM-based WAR from this system peaks ~3-5 WAR (vs. ~6-8 in traditional models),
# so the per-WAR rate is correspondingly higher.
WAR_TO_DOLLAR     = 3_150_000
MAX_MKT_VALUE_PCT = 0.175           # cap at ~$15.4M (~17.5% of $88M cap)
...
def project_war(cur_war, cur_age, fut_age, params):
    ...
    Negative WAR: magnitude shrinks (improves) before peak, grows (worsens) after.
    This preserves the direction of change for both above- and below-average players.
    """
    if cur_war is None:
        return None
    ...
    if cur_war >= 0:
        # Positive: ability grows before peak, declines after
        proj = cur_war * (1.0 + growth_rate) ** grow_yrs * (1.0 - decline_rate) ** decl_yrs
        return max(proj, 0.0)
    else:
        # Negative: magnitude shrinks (improves) before peak, grows (worsens) after
        proj = cur_war * (1.0 - growth_rate) ** grow_yrs * (1.0 + decline_rate) ** decl_yrs
        return min(proj, 0.0)   # stays ≤ 0


def compute_market_value(war, cap_ceiling):
    if war is None or war <= 0:
        return MIN_SALARY
    base = MIN_SALARY + (war * WAR_TO_DOLLAR)
...
df_sk['toi_h'] = df_sk['TOI'] / 60.0          # TOI in minutes → hours
min_toi_h      = 200 / 60.0                    # 200-minute minimum
...
# Season-level league averages (relative to average removes era differences)
...
# WAR proxy = weighted deviation from league average, converted to wins
df_sk['war_proxy'] = (
...
# Build name → player lookup (skaters with birth dates only)
name_lookup: dict = {}
...
# Valid age range for curve fitting
curve_df = df_sk[
...
def age_curve_model(age, peak_age, peak_value, growth_rate, decline_rate):
    """
    Asymmetric hockey age curve.
    Before peak_age: performance grows at growth_rate per year.
    After  peak_age: performance declines at decline_rate per year.
    """
...
def fit_group_curve(group_df, label, p0_peak=26.0):
    """
    Fit age curve for one position group.
    Uses weighted binned means to average out individual quality differences.
    Returns (popt_array, bins_df).
    """
    # Weighted mean WAR proxy per age bin
    agg = (group_df
    ...
    # Initial guess; bounds keep results in hockey-sensible ranges
...
# Survivorship bias compresses growth/decline rates when fitting binned means
# (young stars inflate young-age averages; veterans inflate old-age averages).
# Blend fitted rates with literature values when they fall below sensible floors.
# Peak ages are kept as-is — those are reliable from the data.
LIT = {
    'F': {'grow': 0.08, 'decl': 0.04},
    'D': {'grow': 0.07, 'decl': 0.035},
}
BLEND_FLOOR_GROW  = 0.04   # below 4%/yr → blend 50/50 with literature
BLEND_FLOOR_DECL  = 0.025  # below 2.5%/yr → blend 50/50 with literature
...
# Projection params: keep fitted peak_age and peak_value; use adjusted rates
...
print(f"\n  Projection rates (post-blend, used for trajectories):")
print(f"    Fwd growth {f_grow_proj * 100:.1f}%/yr  decline {f_decl_proj * 100:.1f}%/yr  "
      f"peak {f_peak:.1f}")
print(f"    Def growth {d_grow_proj * 100:.1f}%/yr  decline {d_decl_proj * 100:.1f}%/yr  "
      f"peak {d_peak:.1f}")
...
for _, player in players_df.iterrows():
    ...
    war  = safe_float(player.get('war_total'))
    bd   = player.get('birth_date')
    ci   = player.get('contract_info') or {}

    # Current age: prefer DB column, fall back to birth_year arithmetic
    ...
    # Which curve to use (projection params = fitted peak_age + blended rates)
    pg     = pos_group(pos)
    params = fwd_proj_params if pg == 'F' else def_proj_params
    peak   = f_peak          if pg == 'F' else d_peak

    # Age curve phase relative to fitted peak
    ...
    # Current market value and surplus
    mv = compute_market_value(war, CURRENT_CAP)
    sv = round(mv - cap_hit) if (cap_hit and cap_hit > 0 and war is not None) else None

    # 7-year trajectory (year 0 = current season)
    trajectory = []
    if war is not None and cur_age is not None:
        for y in range(8):
            fut_age  = cur_age + y
            proj_war = project_war(war, cur_age, fut_age, params)
    ...
    results.append({
        ...
        'current_age':     cur_age,
        'cap_hit':         cap_hit,
        'market_value':    mv if war is not None else None,
        'surplus_value':   sv,
...
has_war = res['war_total'].notna()
print(f"  Market value computed for {has_war.sum()}/{len(res)} skaters")

# ── Step 7: Surplus value leaderboards ────────────────────────────────────────
hdr = f"  {'Player':<26} {'Pos':>3} {'WAR':>6} {'Mkt Val':>11} {'Cap Hit':>11} {'Surplus':>12}"
sep = f"  {'-'*26} {'-'*3} {'-'*6} {'-'*11} {'-'*11} {'-'*12}"


def print_row(r):
    war_str = f"{r['war_total']:>6.2f}" if r['war_total'] is not None else f"{'—':>6}"
    print(f"  {r['full_name']:<26} {r['position']:>3} {war_str} "
...
# ── Step 8: Key player trajectories ───────────────────────────────────────────
print("\nSTEP 8 — Key player trajectories")
spotlight_names = [
    'Connor McDavid', 'Nathan MacKinnon', 'Auston Matthews',
    'Connor Bedard', 'Macklin Celebrini', 'Alexander Ovechkin',
]
...
for sname in spotlight_names:
    ...
    if row.empty:
        ...
        continue
    r   = row.iloc[0]
    age_str = f"{int(r['current_age'])}" if r['current_age'] is not None else "—"
    war_str = f"{r['war_total']:.2f}" if r['war_total'] is not None else "—"
    print(f"\n  {sname}  |  Age {age_str}  |  {r['position']}  |  WAR {war_str}"
